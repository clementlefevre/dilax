...
conversionHourMerger = conversion.ConversionHourMerger()
...
def merge_all_observed(datastore):
    merged = _merge_with_counts_observed(datastore)
    merged = _merge_with_conversion(datastore, merged)
    return merged
...
def _merge_with_counts_observed(datastore):
    countsMerger.set_datastore(datastore)
    countsMerger.is_observed = True
    countsMerger.merge_and_clean(datastore.db_manager.sites)
    return countsMerger.merged
...
def _merge_with_conversion(datastore, merged):
    ...
    if datastore.period == 'H':
        conversionHourMerger.set_datastore(datastore)
        conversionHourMerger.merge_and_clean(merged)
        return conversionHourMerger.merged
