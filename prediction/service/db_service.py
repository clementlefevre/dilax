...
def create_counts_sites_weather_holidays_intraday():
    ...
    df_weather_intraday = pd.read_sql(
        'select * from "dwe_ext_weather_meteogroup_intraday"', con=engine)
    df_weather_day = pd.read_sql_query(
        'select * from "dwe_ext_weather_meteogroup_day"', con=engine)
    df_premium = pd.read_sql_query(
        'select * from "dwe_ext_weather_premium"', con=engine)
    ...
    df_holidays = pd.read_sql_query(
        'select * from "dwe_cal_holiday"', con=engine)

    # there are more than one entry per hour per idparent for the intraday
    # data !!!
    groupy_weather_intraday = df_weather_intraday.groupby(
        ['idparent', 'timestamp']).mean()
    groupy_weather_intraday = groupy_weather_intraday.reset_index()

    # merge weather intraday and day tables to retrieve use the coordinates
    # for merging with sites
    df_weather = pd.merge(groupy_weather_intraday, df_weather_day,
                          left_on='idparent', right_on='id', how='inner')

    df_weather['coord'] = df_weather.latitude.apply(lambda x: str(
        round(x, 2))) + ";" + df_weather.longitude.apply(lambda x: str(round(x, 2)))
    df_sites['coord'] = df_sites.latitude.apply(lambda x: str(
        round(x, 2))) + ";" + df_sites.longitude.apply(lambda x: str(round(x, 2)))
    ...
    df_counts['timestamp_hour'] = df_counts.timestamp.apply(
    ...
    df_counts_sites_weather = pd.merge(df_counts[['idbldsite', 'compensatedin', 'timestamp_hour']], df_sites_weather, how='left', left_on=[
                                       'idbldsite', 'timestamp_hour'], right_on=['idbldsite', 'timestamp'], suffixes=['count_', 'weather_'])
    df_counts_sites_weather['date'] = df_counts_sites_weather.timestamp.dt.date

    df = pd.merge(df_counts_sites_weather, df_holidays[['idbldsite', 'day', 'name_en']], left_on=[
                  'idbldsite', 'date'], right_on=['idbldsite', 'day'], how='left')
    df['is_holiday'] = ~df.name_en.isnull() * 1

    columns = ['idbldsite', 'compensatedin', 'timestamp_hour', 'temperature', 'cloudamount_x',
               'weathersituation_x', 'maxtemperature', 'mintemperature', 'sname', 'is_holiday']
...
def create_counts_sites_weather_holidays_day():
    ...
    df_weather_day['coord'] = df_weather_day.latitude.apply(
        round_coordinate) + ";" + df_weather_day.longitude.apply(round_coordinate)
    df_sites['coord'] = df_sites.latitude.apply(
        round_coordinate) + ";" + df_sites.longitude.apply(round_coordinate)
    ...
    df_counts_sites_weather = pd.merge(df_counts[['idbldsite', 'compensatedin', 'date']], df_sites_weather, how='left', left_on=[
