...
def holidays_API_ids():
    ...
    df_sites_holidays_id = pd.merge(
        df_sites, df_geo, left_on='city', right_on='location', how='left')
    # the API key is the field ID
    df_API_ID_list = df_sites_holidays_id.id.value_counts().index
    df_sites_holidays_id.to_csv(
        "holidays/sites_id_holidays_id.csv", encoding='utf-8', sep=';')
...
def create_holidays_table(list_API_ids, year_start=2012):
    ...
    for id in list_API_ids:
        for year in year_range:
            ...
            url = HOLIDAYS_API_ROOT_URL + 'holidays.php?pw=' + \
                urllib.quote(HOLIDAYS_API_PWD) + '&geo=' + \
                id + '&jahr=' + str(year)
            ...
            df_public_holidays = pd.concat(
                [df_public_holidays, public_holidays], axis=0)
            df_school_holidays = pd.concat(
                [df_school_holidays, school_holidays], axis=0)

    df_public_holidays = df_public_holidays.drop_duplicates()
    df_school_holidays = df_school_holidays.drop_duplicates()
    df_public_holidays.to_csv(
        "holidays/public_holidays.csv", encoding='utf-8', sep=';')
    df_school_holidays.to_csv(
        "holidays/school_holidays.csv", encoding='utf-8', sep=';')
